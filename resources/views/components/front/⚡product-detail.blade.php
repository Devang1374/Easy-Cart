...
new class extends Component {
    ...
    public function mount()
    {
        ...
        $this->relatedProducts = product::query()->with('images')->where('category_id', $this->product->category_id)->where('id', '!=', $this->product->id)->where('is_active', true)->take(4)->get();
    }
    ...
    public function addToCart()
    {
        ...
        if ($newQty > $this->product->stock) {
            ...
            Flux::toast(variant: 'danger', heading: 'Stock Limit Reached', text: "Only {$this->product->stock} items available. You alread have $currentQty in Your Cart.");
        ...
        Flux::toast(heading: 'Success', text: 'Product added to cart.');
    }
};
?>
...
<div class="mx-auto max-w-7xl px-6 py-12">
    ...
    <div class="grid gap-10 lg:grid-cols-2">
        ...
        <div class="overflow-hidden rounded-3xl border border-zinc-200 dark:border-zinc-800">
            <img src="{{ $selectedImage }}" class="h-[500px] w-full object-cover">

            <div class="mt-4 flex gap-3 overflow-x-auto">

                @foreach ($product->images as $image)
                    <button wire:click="selectImage('{{ $image->image }}')"
                        class="overflow-hidden rounded-xl border-2 border-transparent hover:border-blue-500">

                        <img src="{{ $image->image }}" class="h-20 w-20 object-cover">
        ...
        <div>
            ...
            @if ($product->stock > 0)
                <div class="mt-6">
                    <span
                        class="rounded-full bg-green-100 px-4 py-2 text-sm font-semibold text-green-700 dark:bg-green-900/30 dark:text-green-400">
                        ✓ In Stock
                    </span>
                </div>
            @else
                <div class="mt-6">
                    <span
                        class="rounded-full bg-red-100 px-4 py-2 text-sm font-semibold text-red-700 dark:bg-red-900/30 dark:text-red-400">
                        ✕ Out of Stock
                    </span>
                </div>
            @endif
            ...
            <div class="mt-8">
                ...
                <div class="flex w-fit items-center rounded-xl border border-zinc-200 dark:border-zinc-800">
                    <button wire:click="decreaseQuantity" class="px-4 py-3">
                        -
                    </button>
                    ...
                    <button wire:click="increaseQuantity" class="px-4 py-3">
                        +
                    </button>
                </div>

                <div class="mt-8">
                    <flux:button wire:click="addToCart" :disabled="$product->stock <= 0" variant="primary"
                        class="w-full">
                        {{ $product->stock > 0 ? 'Add To Cart' : 'Out Of Stock' }}
                    </flux:button>
                </div>

                <a wire:navigate href="{{ route('user/product') }}" class="mt-3 block">
    ...
    <section class="mt-20">
        ...
        <livewire:front.product-review :product="$product" :key="'reviews-' . $product->id" />
        ...
        <section class="mt-20">
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                ...
                @foreach ($relatedProducts as $related)
                    <a href="{{ route('user/productDetails', $related->slug) }}" class="group block">

                        <div
                            class="overflow-hidden rounded-3xl border border-zinc-200 bg-white transition-all duration-300 hover:-translate-y-1 hover:shadow-xl dark:border-zinc-800 dark:bg-zinc-900">

                            @if (isset($related->images[0]))
                                <img src="{{ $related->images[0]->image }}"
