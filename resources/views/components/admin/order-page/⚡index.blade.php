...
new class extends Component {
    ...
    {
        $this->selectedOrder = orderTable::with('items.product.images')->findOrFail($id);
...
<div class="relative w-full flex flex-col gap-5 rounded-xl border border-neutral-200 dark:border-neutral-700 p-5">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-wrap gap-2 overflow-x-auto max-w-full pb-1 sm:pb-0">
            ...
            <flux:button wire:click="$set('status', 'processing')"
                variant="{{ $status === 'processing' ? 'primary' : 'ghost' }}">
                Processing
            </flux:button>

            <flux:button wire:click="$set('status', 'shipped')"
                variant="{{ $status === 'shipped' ? 'primary' : 'ghost' }}">
                Shipped
            </flux:button>

            <flux:button wire:click="$set('status', 'delivered')"
                variant="{{ $status === 'delivered' ? 'primary' : 'ghost' }}">
                Delivered
            </flux:button>

            <flux:button wire:click="$set('status', 'cancelled')"
                ...
                @foreach ($this->orders as $order)
                    <flux:table.row>
                        ...
                        <flux:table.cell>
                            ...
                            @else
                                <span
                                    class="rounded-full bg-yellow-100 px-2 py-1 text-xs font-medium text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400">
                                    Pending
                                </span>
                            @endif
                        ...
                        <flux:table.cell variant="strong">
                            <flux:button wire:click="delete({{ $order['id'] }})" variant="danger" size="sm">
                                Delete</flux:button>
                        </flux:table.cell>

                        <flux:table.cell>
                            <flux:button size="sm" variant="ghost" wire:click="viewOrder({{ $order->id }})">
                                View
                            </flux:button>
                        </flux:table.cell>
            ...
            <div class="space-y-6">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <flux:select wire:change="updateStatus('{{ $selectedOrder->id }}', $event.target.value)"
